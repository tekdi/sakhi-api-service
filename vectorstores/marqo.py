...
class MarqoVectorStore(BaseVectorStore):
    ...
    def add_documents(self, documents=List[Document], fresh_collection: bool = False) -> List[str]:
        if fresh_collection:
            ...
            self.client.create_index(
                self.index_name, settings_dict=self.index_settings)
    ...
    def similarity_search_with_score(self, query: str, index_name: str, k: int = 20) -> List[Tuple[Document, float]]:
        try:
            docsearch = Marqo(self.client, index_name=index_name)
            documents = docsearch.similarity_search_with_score(query, k)
            return documents
        except Exception as e:
            return []
    
    def cache_documents(self, documents: List[Dict[str, str]], index_name: str) -> str:
        """
        ...
        Args:
            ...
            index_name (str): The target collection name.
        ...
        """
        try:
            self.client.create_index(index_name, settings_dict=self.index_settings)
        except Exception as e:
            pass

        response = self.client.index(index_name).add_documents(documents=documents,
                                                         tensor_fields=self.TENSOR_FIELDS)
        if response["errors"]:
            err_msg = (
                f"Error in uploading cache in index range {index_name}"
                f"check Marqo logs."
            )
            raise RuntimeError(err_msg)
        ids = []
